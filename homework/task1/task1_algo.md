...
也就是说，目前只是“非阻塞通信接口改造”，还不是“重叠执行”的最终版本。
...
2. 在 POOL_WORLD 内调用非阻塞点对点通信，收集出完整 array_tot
3. 将 pool 内局部格式的 array_tot 重排为统一布局 array_tot_aux
4. 在 INT_BGROUP 上对 array_tot_aux 做 MPI_Allreduce，结果写回 array_tot
5. 从统一布局的完整结果中，提取当前 rank 对应的本地部分，写回 array_rho
6. 如果 all_ks_run 且 bndpar > 1，再在 BP_WORLD 上进行一次额外归约
```
...
- 现在由 `MPI_Irecv/MPI_Isend/MPI_Waitall` 显式实现
...
需要说明的是：  
第三阶段版本不一定在所有环境下都比 `MPI_Allgatherv` 更快，因为 MPI 内部可能已经对 collective 做了很强优化。  
...
### 11.1 第四阶段：使用 `MPI_Waitsome` 实现边接收边重排

当前版本使用 `MPI_Waitall`，所有消息到齐后才执行重排。  
后续可改为：

- 哪个 rank 的数据先到
- 就先把该 rank 对应的数据块重排到统一布局中

这样可以真正实现通信与计算重叠。

### 11.2 第五阶段：改造为 root-only gather/reduce/scatter
