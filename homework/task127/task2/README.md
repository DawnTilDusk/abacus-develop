...
| [source/source_estate/test_omp/charge_omp_test.cpp](file:///root/homework/abacus-develop/source/source_estate/test_omp/charge_omp_test.cpp) | 新增 + 阶段 4 扩展 | `_OPENMP` 健全性测试 + 两个 reorder 对比测试 |
| [source/source_estate/module_charge/charge_mpi.cpp](file:///root/homework/abacus-develop/source/source_estate/module_charge/charge_mpi.cpp) | `reorder_pool_rank_to_uniform` 内加 `#pragma omp parallel for collapse(2)`（阶段 4），含 `#ifdef _OPENMP` 保护 | 题目一 target 无 `_OPENMP` → 行为不变 |
...
- 阶段 5：在 `extract_uniform_to_local` 加同样的 `#pragma omp parallel for collapse(2)` + `#ifdef _OPENMP` 保护
- 阶段 6：补 `ncxy=1` / `numz=[0,1]` 等边界单元测试，并加 extract 串并行对比
